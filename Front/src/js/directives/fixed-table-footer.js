...
angular.module('app').directive('fixFoot', fixHead);
...
function fixHead($compile, $window) {
  ...
  function compile(tElement) {
    ...
    header.clone.removeAttr('fix-foot').removeAttr('ng-if');
    ...
    return function postLink(scope) {
      ...
      $(scrollContainer[0]).after(table.clone.append(header.clone)[0]);
      // scrollContainer[0].parentNode.insertBefore(table.clone.append(header.clone)[0], scrollContainer[0])
      ...
      function getCells(node) {
        return Array.prototype.map.call(node.find('td'), function (cell) {
          return jQLite(cell);
        });
      }
