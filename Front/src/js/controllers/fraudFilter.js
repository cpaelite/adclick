(function () {
  ...
  angular.module('app')
    .controller('fraudFilterCtrl', [
      '$scope', '$mdDialog', 'BlackList', 'FraudFilterRule', 'FraudFilterLog', 'DateRangeUtil', 'FraudFilterLogDetail', '$auth', '$q', '$state', 'toastr', '$cookies',
      fraudFilterCtrl
    ]);

  function fraudFilterCtrl($scope,  $mdDialog, BlackList, FraudFilterRule, FraudFilterLog, DateRangeUtil, FraudFilterLogDetail, $auth, $q, $state, toastr, $cookies) {
    ...
    $scope.viewLogs = function(tr){
      $mdDialog.show({
        clickOutsideToClose: false,
        controller: ['$scope', '$mdDialog', 'FraudFilterLogDetail', ruleLogCtrl],
        controllerAs: 'ctrl',
        focusOnOpen: false,
        locals: {data:tr},
        bindToController: true,
    ...
    $scope.getRuleList = function(){
      FraudFilterRule.get($scope.query,function(result){
    ...
    $scope.ruleStatusChange = function(index){
      ...
      FraudFilterRule.save($scope.list.rules[index]);
    };
    ...
    $scope.getLogList = function(){
      FraudFilterLog.get($scope.queryLog,function(result){
    ...
    $scope.deleteRule = function(id){
      $mdDialog.show({
        clickOutsideToClose: true,
        escapeToClose: false,
        controller: ['$scope', '$mdDialog', 'FraudFilterRule', deleteRuleCtrl],
        controllerAs: 'ctrl',
        focusOnOpen: false,
        locals: {id: id},
        bindToController: true,
  ...
  function ruleLogCtrl($scope, $mdDialog, FraudFilterLogDetail){
    this.cancel = $mdDialog.cancel;
    FraudFilterLogDetail.get({id:this.data.id},function(result){
