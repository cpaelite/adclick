...
router.get('/api/traffic/tpl', function(req, res, next) {
    pool['m1'].getConnection(function(err, connection) {
        ...
        connection.query(
            "select `id`,`name`,`postbackUrl`,`pixelRedirectUrl` ,`externalId`, `cost`,`params`,`campaignId`,`websiteId`, `apiReport`, `apiParams` from TemplateTrafficSource where `deleted`=? order by `order` ASC", [
                0
            ],
