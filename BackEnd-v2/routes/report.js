...
async function IPReport(req) {
  ...
  try {
    ...
    let sql = `select CountryCode as country,IP as ip,CampaignID as campaignId,sum(Impressions) as impressions,sum(Visits) as visits,sum(Clicks) as clicks,sum(Conversions) as conversions,
                  round(sum(Cost/1000000),2) as cost,
                  round(sum(Revenue/1000000),2) as revenue,
