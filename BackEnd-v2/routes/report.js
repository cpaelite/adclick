        ...
        var _ref = _asyncToGenerator(function* () {
            try {
                ...
                if (value.groupBy == "campaign") {
                    ...
                    result = yield landerReport(value, connection);
                } else if (value.groupBy == "flow") {
                    result = yield flowReport(value, connection);
                } else if (value.groupBy == "affiliateNetwork") {
                    result = yield affiliateReport(value, connection);
                } else if (value.groupBy == "trafficSource") {
                    result = yield trafficReport(value, connection);
                }
                connection.release();
                res.json({
                    status: 1,
        ...
        const start = (() => {
            var _ref6 = _asyncToGenerator(function* () {
                try {
                    let offset = (value.offset - 1) * value.limit;
                    let limit = value.limit;
                    let sql = "select  t.`id`,t.`name` as `name`,t.`hash` as `hash` ,t.`url` ,t.`impPixelUrl` ,t.`country` ," + "t.`trafficSourceName` ,t.`costModel`,t.`cpcValue` as `cpc`,t.`cpaValue` as `cpa`,t.`cpmValue` as `cpm`," + "t.`redirectMode` as `redirect`," + "ifnull(a.`Impressions`,0) as `impressions`," + "ifnull(a.`Visits`,0) as `visits`,ifnull(a.`Clicks`,0) as `clicks`," + "ifnull(a.`Conversions`,0) as `conversions`,Round(ifnull(a.`Revenue`,0),2) as `revenue`," + "Round(ifnull(a.`Cost`,0),2)  as `cost`," + "Round(ifnull(a.`profit`,0),2)  as `profit`," + "Round(ifnull(a.`cpv`,0),4) as `cpv`," + "Round(ifnull(a.`ictr`,0)*100,2) as `ictr`," + "Round(ifnull(a.`ctr`,0)*100,2) as `ctr`," + "Round(ifnull(a.`cr`,0)*100,2) as `cr`," + "Round(ifnull(a.`cv`,0)*100,2)  as `cv`," + "Round(ifnull(a.`roi`,0)*100,2) as `roi`," + "Round(ifnull(a.`epv`,0)*100,4) as `epv`," + "Round(ifnull(a.`epc`,0)*100,2) as `epc`," + "Round(ifnull(a.`ap`,0)*100,2) as `ap` " + "from `TrackingCampaign` t left join  " + "(select sum(`Impressions`) as `Impressions`,sum(`Visits`) as `Visits`,sum(`Clicks`) as `Clicks`,sum(`Conversions`) as `Conversions`,sum(`Revenue`/1000000) as `Revenue`,sum(`Cost`/1000000) as `Cost` ,`CampaignID`," + "sum(`Revenue`/1000000)-sum(`Cost`/1000000) as `profit` , " + "sum(`Cost`/1000000)/sum(`Impressions`) as `cpv`," + "sum(`Visits`)/sum(`Impressions`) as `ictr`," + "sum(`Clicks`)/sum(`Visits`) as `ctr`," + "sum(`Conversions`)/sum(`Clicks`) as `cr`," + "sum(`Conversions`)/sum(`Visits`) as `cv`," + "sum(`Revenue`/1000000)/sum(`Cost`/1000000) as `roi`," + "sum(`Revenue`/1000000)/sum(`Visits`) as `epv`," + "sum(`Revenue`/1000000)/sum(`Clicks`) as `epc`," + "sum(`Revenue`/1000000)/sum(`Conversions`) as `ap`  from  `AdStatis`";

                    sql += " where `UserID`=" + value.userId + " and  `Timestamp` >=" + "UNIX_TIMESTAMP(CONVERT_TZ('" + value.from + "', '+00:00','" + value.tz + "')) and `Timestamp` <=" + "UNIX_TIMESTAMP(CONVERT_TZ('" + value.to + "', '+00:00','" + value.tz + "')) group by `CampaignID` ) a  on a.`CampaignID`= t.`id` where t.`userId`= " + value.userId;
