...
router.get('/api/automated/rules/:id', async function (req, res, next) {
    ...
    try {
        ...
        let sql = `select id,name,campaigns,dimension,timeSpan,conditions,schedule,status from SuddenChangeRule where id= ? and userId = ?`;
...
router.get('/api/automated/rules', async function (req, res, next) {
    ...
    try {
        ...
        let sql = `slect id,name,campaigns,dimension,timeSpan,status from SuddenChangeRule where userId =? and deleted=0 ${filter} limit ?,?`;
...
router.get('/api/automated/logs', async function (req, res, next) {
    ...
    try {
        ...
        let sql = `select log.id as id ,DATE_FORMAT(FROM_UNIXTIME(log.timeStamp), "%Y-%d-%m %H:%i:%s") as time,rule.name as name,log.dimension as dimension  
                  from SuddenChangeLog log inner join SuddenChangeRule rule on log.ruleId = rule.id  where rule.userId =?  limit ?,?`;
        let Result = await common.query(sql, [value.userId, value.offset, value.limit], connection);
        return res.json({
            status: 1,
            message: 'success',
            data: {
