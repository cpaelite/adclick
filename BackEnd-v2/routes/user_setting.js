...
router.post('/api/password', async function(req, res, next) {
  ...
  try {
    ...
    let result = await query(
      "select `password`,`idText` from User where `id`= ? ", [value.userId],
      connection);
    if (result.length) {
      if (md5(value.oldpassword) == result[0].password) {
        await query("update User set `password`= ? where `id`= ? ", [md5(
          value.newpassword), value.userId], connection);
        return res.json({
          status: 1,
          message: 'success'
        });
...
router.post('/api/email', async function(req, res, next) {
  ...
  try {
    ...
    connection = await common.getConnection();
    //check email 是否存在
    let UserResult = await query("select id from User where `email`= ? ", [
      value.email
    ], connection);
...
router.get('/api/referrals', async function(req, res, next) {
  ...
  try {
    ...
    let tmp =
      `select a.idText as referredUserId, FROM_UNIXTIME(a.acquired,'%d-%m-%Y %H:%i') as acquired,
...
left join User user on user.id= ref.referredUserId  where mis.createdAt><%=time%> and ref.userId= <%=userId%> group by ref.referredUserId) b on  a.idText = b.idText`
    ...
    let countsql =
      "select COUNT(*) as `count`,sum(recentCommission) as recentCommission,sum(totalCommission) as totalCommission from ((" +
      listSql + ") as T)";

    listSql += " order by " + order + " " + dir + " limit " + offset + "," +
      limit;

    let result = await Promise.all([query(listSql, [], connection), query(
      countsql, [], connection)]);
    res.json({
      status: 1,
      message: 'succes',
...
router.get('/api/domains', async function(req, res, next) {
  ...
  try {
    ...
    let sql =
      "select `domain` as address,`main`,`customize`,`verified` from UserDomain where `userId`= ? and `deleted`=0 ";
...
router.post('/api/domains', async function(req, res, next) {
  ...
  try {
    ...
    connection = await common.getConnection();
    await common.query("delete from  UserDomain  where `userId`= ? ", [
      value.userId
    ], connection);
    if (verfiedMainDomain) {
      await common.query(
        'update UserDomain set `verified`= 0 where `domain`= ? and `customize` = 1 ', [
          verfiedMainDomain.address
        ], connection);
    }
    ...
    //redis publish
    redisPool.publish(setting.redis.channel, value.userId + ".update.user." +
      value.userId);
    delete value.userId;
    return res.json({
      status: 1,
...
router.post('/api/invitation', async function(req, res, next) {
  var schema = Joi.object().keys({
    userId: Joi.number().required(),
    idText: Joi.string().required(),
    invitationEmail: Joi.array().items(Joi.string().email()).required()
      .min(1),
  ...
  try {
    ...
    for (let index = 0; index < value.invitationEmail.length; index++) {
      let invatationSlice = await common.query(
        "select `id`,`groupId`,`inviteeEmail`,`status`,`code` from GroupInvitation where `deleted`= 0 and `groupId`=? and `inviteeEmail`=? ", [
          value.groupId, value.invitationEmail[index]
        ], connection);
      //存在未接受邀请的重新发送
      if (invatationSlice.length) {
        if (invatationSlice[0].status != 1) {
          await common.query(
            "update GroupInvitation set `status`= 0 ,`inviteeTime`= unix_timestamp(now()) where `id`= ? ", [
              invatationSlice[0].id
            ], connection);
          sendContext.push({
        ...
        let code = uuidV4();
        await common.query(
          "insert into GroupInvitation (`userId`,`groupId`,`inviteeEmail`,`inviteeTime`,`code`) values(?,?,?,unix_timestamp(now()),?)", [
            value.userId, value.groupId, value.invitationEmail[index],
            code
          ], connection);
        sendContext.push({
          email: value.invitationEmail[index],
    ...
    let results = await common.query(
      'select `id` ,`inviteeEmail` as email,FROM_UNIXTIME( `inviteeTime`, \"%d-%m-%Y\") as lastDate,`status` from GroupInvitation where (`status`= 0 or `status`= 1) and  `deleted`= 0 and `groupId`=? and `userId`= ? ', [
        value.groupId, value.userId
      ], connection)
    if (beginTransaction) {
...
router.delete('/api/invitation/:id', async function(req, res, next) {
  ...
  try {
    ...
    let value = await common.validate(req.query, schema);
    connection = await common.getConnection();
    let updateGI = common.query(
      'update GroupInvitation set `status` = ? where `id`=? and `userId`= ?', [
        3, value.id, value.userId
      ], connection);
    let selectUser = common.query(
      'select gi.`groupId`,gi.`inviteeEmail`,user.`id` from GroupInvitation gi  inner join User user on user.`email`= gi.`inviteeEmail` where gi.`userId`=? and gi.`id`= ? ', [
    ...
    if (Results[0].length) {
      await common.query(
        'delete from UserGroup where `groupId`=? and `userId`= ? and `role`= 1', [
          Results[0][0].groupId, Results[0][0].id
        ], connection);
    }
...
router.get('/api/setup', async function(req, res, next) {
  ...
  try {
    ...
    let value = await common.validate(req.query, schema);
    connection = await common.getConnection();
    let domainResult = await common.query(
...
router.get('/api/user/plan', async function(req, res, next) {
  ...
  try {
    ...
    connection = await common.getConnection();
    let sql =
      `select DATE_FORMAT(FROM_UNIXTIME(bill.planStart), "%m/%d/%Y") as \`from\` ,
