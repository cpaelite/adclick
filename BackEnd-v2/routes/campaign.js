...
const start = (() => {
    var _ref = _asyncToGenerator(function* (data, schema) {
        ...
        let ResultError;
        try {
            let value = yield validate(data, schema);
            ...
            try {
                ...
                if (value.id) {
                    yield updateCampaign(value, connection);
                } else {
                    campResult = yield insertCampaign(value, connection);
                }

                //Flow
                if (value.flow && !value.flow.id) {
                    flowResult = yield insertFlow(value, connection);
                } else if (value.flow && value.flow.id) {
                    yield updateFlow(value, connection);
                }
                console.log(11111);
                ...
                //删除所有tags
                yield updateTags(value.userId, campaignId, 1, connection);

                //campain Tags
                if (value.tags && value.tags.length > 0) {
                    if (value.tags && value.tags.length > 0) {
                        for (let index = 0; index < value.tags.length; index++) {
                            yield insertTags(value.userId, campaignId, value.tags[index], 1, connection);
                        }
                    }
                }
                    ...
                    for (let i = 0; i < value.flow.rules.length; i++) {
                        try {
                            ...
                            if (!value.flow.rules[i].id) {
                                ruleResult = yield insetRule(value.userId, value.flow.rules[i], connection);
                                yield insertRule2Flow(ruleResult.insertId, flowId, value.flow.rules[i].rule2flow, connection);
                            } else {
                                yield updateRule(value.userId, value.flow.rules[i], connection);
                                yield updateRule2Flow(value.flow.rules[i].rule2flow, value.flow.rules[i].id, flowId, connection);
                            }
                            ...
                            if (value.flow.rules[i].paths && value.flow.rules[i].paths.length > 0) {
                                for (let j = 0; j < value.flow.rules[i].paths.length; j++) {
                                    ...
                                    if (!value.flow.rules[i].paths[j].id) {
                                        pathResult = yield insertPath(value.userId, value.flow.rules[i].paths[j], connection);
                                        yield insertPath2Rule(pathResult.insertId, ruleId, value.flow.rules[i].paths[j].weight, value.flow.rules[i].paths[j].path2rule, connection);
                                    } else {
                                        yield updatePath(value.userId, value.flow.rules[i].paths[j], connection);
                                        yield updatePath2Rule(value.flow.rules[i].paths[j].id, value.flow.rules[i].id, value.flow.rules[i].paths[j].weight, value.flow.rules[i].paths[j].path2rule, connection);
                                    }
                                    ...
                                    if (value.flow.rules[i].paths[j].landers && value.flow.rules[i].paths[j].landers.length > 0) {
                                        for (let k = 0; k < value.flow.rules[i].paths[j].landers.length; k++) {
                                            ...
                                            if (!value.flow.rules[i].paths[j].landers[k].id) {
                                                landerResult = yield insertLander(value.userId, value.flow.rules[i].paths[j].landers[k], connection);
                                                yield insertLander2Path(landerResult.insertId, pathId, value.flow.rules[i].paths[j].landers[k].weight, connection);
                                            } else {
                                                yield updateLander(value.userId, value.flow.rules[i].paths[j].landers[k], connection);
                                                yield updateLander2Path(value.flow.rules[i].paths[j].landers[k].id, pathId, value.flow.rules[i].paths[j].landers[k].weight, connection);
                                            }
                                            ...
                                            yield updateTags(value.userId, landerId, 2, connection);
                                            ...
                                            if (value.flow.rules[i].paths[j].landers[k].tags && value.flow.rules[i].paths[j].landers[k].tags.length > 0) {
                                                for (let q = 0; q < value.flow.rules[i].paths[j].landers[k].tags.length; q++) {
                                                    ...
                                                    yield insertTags(value.userId, landerId, value.flow.rules[i].paths[j].landers[k].tags[q], 2, connection);
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    if (value.flow.rules[i].paths[j].offers && value.flow.rules[i].paths[j].offers.length > 0) {
                                        for (let z = 0; z < value.flow.rules[i].paths[j].offers.length; z++) {
                                            ...
                                            if (!value.flow.rules[i].paths[j].offers[z].id) {
                                                offerResult = yield insertOffer(value.userId, value.flow.rules[i].paths[j].offers[z], connection);
                                                yield insertOffer2Path(offerResult.insertId, pathId, value.flow.rules[i].paths[j].offers[z].weight, connection);
                                            } else {
                                                yield updateOffer(value.userId, value.flow.rules[i].paths[j].offers[z], connection);
                                                yield updateOffer2Path(value.flow.rules[i].paths[j].offers[z].id, pathId, value.flow.rules[i].paths[j].offers[z].weight, connection);
                                            }
                                            ...
                                            //删除所有offer tags
                                            yield updateTags(value.userId, offerId, 3, connection);
                                            //offer tags 
                                            if (value.flow.rules[i].paths[j].offers[z].tags && value.flow.rules[i].paths[j].offers[z].tags.length > 0) {
                                                for (let p = 0; p < value.flow.rules[i].paths[j].offers[z].tags.length; p++) {
                                                    yield insertTags(value.userId, offerId, value.flow.rules[i].paths[j].offers[z].tags[p], 3, connection);
                                                }
                                            }
                                        }
                                    }
