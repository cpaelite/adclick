    ...
    return new Promise(function (resolve, reject) {
        ...
        const start = async()=> {
            try {
                ...
                let sql;
                sql = "select ads.UserID,ads.Language,ads.Model,ads.Country,ads.City,ads.Region,ads.ISP,ads.MobileCarrier,ads.Domain,ads.DeviceType,ads.Brand,ads.OS,ads.OSVersion,ads.Browser,ads.BrowserVersion,ads.ConnectionType,ads.Timestamp,sum(ads.Visits) as Visits,sum(ads.Clicks) as Clicks, sum(ads.Conversions) as Conversions, sum(ads.Cost) as Cost, sum(ads.Revenue) as Revenue,sum(ads.Impressions) as Impressions,ads.KeysMD5,ads.V1,ads.V2,ads.V3,ads.V4,ads.V5,ads.V6,ads.V7,ads.V8,ads.V9,ads.V10," +
                ...
                sql += " where ads.UserID=" + value.userId + " and  ads.`Timestamp` >=" + "UNIX_TIMESTAMP(CONVERT_TZ('" + value.from + "', '+00:00','" + value.tz + "')) and ads.`Timestamp` <=" +
